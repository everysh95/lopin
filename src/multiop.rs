...
#[async_trait]
impl<ST: Clone + Send + Sync + 'static, DT: Clone + Send + Sync + 'static> RawConverter<ST, DT> for ConditionedConverter<ST, DT>
{
    ...
    async fn from(&self, dist: DT) -> Option<ST> {
        match self.converter.from(dist).await {
            Some(src) => if self.condition.validation(src.clone()).await {
                Some(src)
            } else {
                None
            },
...
#[async_trait]
impl<ST: Clone + Send + Sync + 'static, DT: Clone + Send + Sync + 'static> RawConverter<ST, DT> for ConditionedConverterR<ST, DT>
{
    async fn to(&self, src: ST) -> Option<DT> {
        match self.converter.to(src).await {
            Some(dist) => if self.condition.validation(dist.clone()).await {
                Some(dist)
            } else {
                None
            },
    ...
    async fn from(&self, dist: DT) -> Option<ST> {
        if self.condition.validation(dist.clone()).await {
            self.converter.from(dist).await
        } else {
            None
        }
    }
